...
flowchart TB
    ...
    subgraph Tier3_ANLY["Tier3 分析テーブル"]
...
### WK_RT_EC_MMBR（EC会員ワーク）
**生成ジョブ**: MRT_WK_EC_100.sql

| Tier2ソーステーブル | 説明 |
|---------------------|------|
| V_COR_MMBR_EC | EC会員マスタ |
| V_COR_CUST_WEB_PRPY | 顧客Web属性 |

```mermaid
flowchart TB
    subgraph Tier2["Tier2 マスタビュー"]
        V_COR_MMBR_EC["V_COR_MMBR_EC<br/>EC会員マスタ"]
        V_COR_CUST_WEB_PRPY["V_COR_CUST_WEB_PRPY<br/>顧客Web属性"]
    end
    subgraph Tier3["Tier3 ワークテーブル"]
        WK_RT_EC_MMBR["WK_RT_EC_MMBR<br/>EC会員ワーク"]
    end
    subgraph Tier3_ANLY["Tier3 分析テーブル"]
        RT_SALE_ANLY["RT_SALE_ANLY<br/>販売分析マート"]
    end
    V_COR_MMBR_EC --> WK_RT_EC_MMBR
    V_COR_CUST_WEB_PRPY --> WK_RT_EC_MMBR
    WK_RT_EC_MMBR --> RT_SALE_ANLY
```
