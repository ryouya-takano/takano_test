...
> **注記**: ER図中の属性説明に「（PI）」と記載されている項目は、TeradataのPRIMARY INDEXを表します。
...
erDiagram
    %% 最上位ビュー
    V_BIA_BL_SALE_MART {
        DECIMAL itgt_cust_id "統合顧客ID（PI）"
    ...
    WK_SALE_ANLY {
        DECIMAL busn_cd "事業コード（PI）"
        DECIMAL cust_no "顧客番号（PI）"
    }

    WK_RT_GODS {
        DECIMAL busn_cd "事業コード（PI）"
        VARCHAR exec_cd "実行コード（PI）"
        INTEGER new_prm_gods "新プロモ商品（PI）"
    }
    ...
    WK_HANPU_DTLS {
        DECIMAL busn_cd "事業コード（PI）"
        DECIMAL cust_no "顧客番号（PI）"
        DECIMAL ord_no "受注番号（PI）"
        DECIMAL ord_line_no "受注行番号（PI）"
    }
    ...
    V_COR_MM_STK {
        DECIMAL busn_cd "事業コード（PI）"
        VARCHAR gods_cd "商品コード（PI）"
        DECIMAL gods_srno "商品連番（PI）"
        DECIMAL yymm "年月（PI）"
...
V_COR_EXCD_GODSを駆動表とし、V_COR_PRM_GODS_S1_BBOBをハブとして商品マスタ系各種ビューをJOINし、WK_RT_GODSを生成しています。
...
erDiagram
    ...
    %% Tier2 ハブテーブル（V_COR_EXCD_GODSにJOIN）
    ...
    %% Tier2 商品マスタ系（V_COR_PRM_GODS_S1_BBOBにJOIN）
    V_COR_PRM_GODS {
        DECIMAL busn_cd "事業コード"
        VARCHAR prm_cd "プロモーションコード"
        INTEGER new_prm_gods "新プロモ商品"
    ...
    %% Tier2 オンライン商品系（V_COR_EXCD_GODSにJOIN）
    V_COR_OL_GODS_DTLS {
        DECIMAL busn_cd "事業コード"
        INTEGER new_prm_gods "新プロモ商品"
    ...
    %% Tier2 サイズマスタ（V_COR_GODS_S1_BBOBにJOIN）
    RT_APRL_SIZE {
        VARCHAR size_cd "サイズコード"
...
erDiagram
    ...
    %% Tier2からWK_HANPU_DTLSへ（MRT_WK_HNP_100.sql）
    ...
    V_COR_REQ_HDR ||--o{ WK_HANPU_DTLS : "busn_cd, cust_no, ord_no"
    V_COR_ORDR_DTLS ||--o{ WK_HANPU_DTLS : "busn_cd, cust_no, ord_no, ord_line_no"

    %% Tier2からWK_RT_PRID_DTLSへ（MRT_WK_PRID_100.sql）
    ...
    V_COR_ORDR_DTLS ||--o{ WK_RT_PRID_DTLS : "busn_cd, cust_no, ord_no, ord_line_no"
```
